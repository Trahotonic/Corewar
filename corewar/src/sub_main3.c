...
t_player	*ft_player_create(char *champ, int num)
{
	...
	if (ft_strstr2(champ, ".cor"))
	{
		...
		if (tmp->fd == -1)
			exit(ft_printf(2, "Invalid champion provided\n"));
		tmp->next = NULL;
		tmp->last_alive = 0;
		tmp->live_count = 0;
		tmp->live_count_p = 0;
		...
		exit(ft_printf(2, "Invalid champion\n"));
	return (tmp);
}
	...
	if (argv[*n])
	{
		while (argv[*n][i])
		{
			if (!ft_isdigit(argv[*n][i]))
				exit(ft_printf(2, "invalid arguments\n"));
			++i;
		}
...
void		ft_check_flag_vi(char **argv, int *n, t_arg_flags *flags)
{
	...
	if (argv[*n])
	{
		...
		{
			if (!ft_isdigit(argv[*n][i]))
				exit(ft_printf(2, "invalid iteration\n"));
			++i;
		}
	...
	else
	{
		...
		while ((tmp)->next)
		{
			if (tmp->player_number == ft_atoi(argv[n]))
				exit(ft_printf(2, "Error same number in players\n"));
			tmp = (tmp)->next;
			i++;
		}
		if (tmp->player_number == ft_atoi(argv[n]))
